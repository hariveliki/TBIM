...
def Plot_CellsByKey(
    ...
    if data is not None:
        ...
        print("Warning: DataByTopology returned None. Check if cellComplex is valid.")

    # Str Values
    if keyType == "str":
        # Values
        values = [
            Dictionary.ValueAtKey(Topology.Dictionary(_Cell), key)
        ...
        for _Cell in CellComplex.Cells(cellComplex):
            ...
            color = Color.ByValueInRange(
                value=valueNum, minValue=0, maxValue=maxValue, colorScale="turbo"
            )
            colorStr = (
        ...
        # Legend
        legend_traces = [
            Plot_LegendTrace(color, label) for color, label in color_labels
        ]
        legend_traces.insert(
            0,
            go.Scatter(
                x=[None],
                y=[None],
    ...
    if keyType == "num":
        ...
        for _Cell in CellComplex.Cells(cellComplex):
            value = Dictionary.ValueAtKey(Topology.Dictionary(_Cell), key)
            color = Color.ByValueInRange(
                value=value, minValue=0, maxValue=maxValue, colorScale=colorScale
            )
            colorStr = (
                "rgb(" + str(color[0]) + "," + str(color[1]) + "," + str(color[2]) + ")"
            )
...
def Plot_FacesByKey(
    ...
    if keyType == "str":
        # Values
        values = [
            [Dictionary.ValueAtKey(Topology.Dictionary(face), key)][0] for face in faces
        ]
        values = sorted(list(set(values)))
        maxValue = len(values)
        values_numbers = [i for i in range(maxValue)]
...
def Plot_AperturesByKey(
    ...
    dataList = []
    data = Plotly.DataByTopology(
        cellComplex, faceOpacity=0.2, showVertexLegend=False, showEdgeLegend=False
    )
    dataList += data
    # Values
    ...
    for _Face in _Faces:
        for _Aperture in Topology.Apertures(_Face):
            ...
            _Face_BetweenInformedSpace_Prop = Dictionary.ValueAtKey(
                _Face_Dictionary, _Face_PropName
            )
    ...
    _Face_BetweenInformedSpace_Props = sorted(
        list(set(_Face_BetweenInformedSpace_Props))
    )
    _Face_BetweenInformedSpace_Props_Number = [
        i for i in range(len(_Face_BetweenInformedSpace_Props))
    ]
    # Colors
    color_labels = []
    for i, _Face_BetweenInformedSpace_Prop in enumerate(
        _Face_BetweenInformedSpace_Props
    ):
        ...
        color = Color.ByValueInRange(
            value=value,
            minValue=0,
            maxValue=len(_Face_BetweenInformedSpace_Props),
            colorScale="rainbow",
        )
        colorStr = (
            "rgb(" + str(color[0]) + "," + str(color[1]) + "," + str(color[2]) + ")"
    ...
    for _Face in _Apertures:
        ...
        if _Face_BetweenInformedSpace_Prop in _Face_BetweenInformedSpace_Props:
            index = _Face_BetweenInformedSpace_Props.index(
                _Face_BetweenInformedSpace_Prop
            )
    ...
    legend_traces.insert(
        ...
        go.Scatter(
            ...
            mode="markers",
            marker=dict(size=0.1),
            name="<b>" + _Face_PropName[0:].replace("_pSP_", ": ") + "</b>",
            showlegend=True,
        ),
    )
    dataList.extend(legend_traces)
...
def Plot_AdjacencyGraph(
    ...
    data02 = Plotly.DataByTopology(
        cellComplex, faceOpacity=0.2, edgeColor="gray", showVertices=False
    )
    data01 = Plotly.DataByGraph(
        g,
        vertexLabelKey="type",
        vertexGroupKey="type",
        vertexGroups=[0, 1, 2, 3, 4, 5],
        colorScale="turbo",
